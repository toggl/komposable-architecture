...
class EffectDebounceTests {
    @Test
    fun `effects debounce on stores`() =
        runTest {
            val reducer =
                Reducer<Int, String> { state, action ->
                    when (action) {
                        "tap_add" ->
                            ReduceResult(
                                state,
    ...
    @Test
    fun `effects do not affect their internal flows`() =
        runTest {
            val someState = MutableStateFlow(emptySet<Int>())
