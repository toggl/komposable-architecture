...
class StoreStateTests : StoreCoroutineTest() {
    @ExperimentalTime
    @Test
    fun `state is emitted in batches after all sent actions were reduced`() =
        runTest {
    ...
    @ExperimentalTime
    @Test
    fun `new state is emitted only when it's different than the previous one`() =
        runTest {
            testStore.state.test {
                testStore.send(
                    listOf(
    ...
    @ExperimentalTime
    @Test
    fun `effects emit new state only after the result state of previously sent actions was emitted`() =
        runTest {
            testStore.state.test {
                testStore.send(
                    listOf(
