...
class StoreReducerTests : StoreCoroutineTest() {
    @Test
    fun `reducer shouldn't be called if the list of sent actions is empty`() =
        runTest {
    ...
    @Test
    fun `reducer should be called exactly once if one action is sent`() =
        runTest {
    ...
    @Test
    fun `effects with multiple actions are processed correctly`() =
        runTest {
            val flowEffect =
                spyk(
                    TestEffect(
