...
class TestStoreTests {
    open class BaseTestStoreTest {
        ...
        val nonExhaustiveTestConfig =
            NonExhaustiveTestConfig(
                dispatcherProvider,
                testCoroutineScope,
                JavaLogger(Logger.getLogger("TestStore")),
                reflectionHandler = JvmReflectionHandler(),
    ...
    inner class MergeTests : BaseTestStoreTest() {
        private val reducer =
            Reducer<Any, MergeTestsAction> { state, action ->
                when (action) {
                    MergeTestsAction.A ->
                        state.withEffect(
                            Effect.merge(
        ...
        @Test
        fun `merged flows are merged together as they are reduced`() =
            runTest {
    ...
    inner class AsyncEffects : BaseTestStoreTest() {
        @Test
        fun `async effects are received`() =
            runTest {
                val reducer =
                    Reducer<Int, AsyncEffectsAction> { state, action ->
                        when (action) {
                            is AsyncEffectsAction.Tap ->
                                state.withEffect(
    ...
    inner class ActionMatching : BaseTestStoreTest() {
        @Test
        fun `matching predicates are executed`() =
            runTest {
                val reducer =
    ...
    inner class StateAccess : BaseTestStoreTest() {
        @Test
        fun `test store state is updated after assertion`() =
            runTest {
                val initialState = 0
                val reducer =
    ...
    inner class ExhaustivityTests : BaseTestStoreTest() {
        ...
        private val reducer =
            Reducer<ExhaustivityTestState, ExhaustivityTestAction> { state, action ->
                when (action) {
                    ...
                    ExhaustivityTestAction.IncrementTap ->
                        state.withSuspendEffect {
                            delay(1001)
                            ExhaustivityTestAction.Increment
                        }
                }
        ...
        inner class NonExhaustive {
            @Test
            fun `partial changes are verified`() =
                runTest {
            ...
            @Test
            fun `cumulative changes can be verified at the end`() =
                runTest {
                    reducer.test(
                        initialState,
            ...
            @Test
            fun `finish does not wait for effects completion`() =
                runTest {
                    reducer.test(initialState, nonExhaustiveTestConfig) {
                        send(ExhaustivityTestAction.IncrementTap)
        ...
        inner class Exhaustive {
            @Test
            fun `must assert everything`() =
                runTest {
                    shouldThrow<AssertionError> {
                        // did not wait for effects to finish
                        reducer.test(initialState, exhaustiveConfig) {
